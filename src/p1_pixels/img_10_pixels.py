...
class PixelGroup_Glyph:
    ...
    def pixels_convex_hull_collect(self) -> list[str]:
        ...
        if not errors:
            ...
            for x in range(self.x_min, self.x_max+1):
                for y in range(self.y_min, self.y_max+1):
                    # print(f"pixels collect in convex hull: {x:>3} {y:>3}")
...
def pixelGroup_matrix_representation_convert_to_str__forHumanReadingInTerminal(
    ...
    for yRelativeInMatrix, row in enumerate(matrix_representation):
        ...
        for (_xAbs, yAbs, pixelRepresentation) in row:
            ...
            if display:
                rowDisplayed.append(charPlaceholderActive)
                counterDisplayed += 1
            else:
                rowDisplayed.append(charPlaceholderInactive)
